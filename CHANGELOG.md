...
- Docs: 重写根目录 `README.md`，调整为总览型项目文档，补齐项目定位、技术栈、仓库结构、核心能力、快速开始与子项目指引。

## 2026/04/13

- Fixed: 修复仪表盘 30 秒自动刷新仍使用初始筛选条件的问题，手动刷新与定时刷新都会沿用当前选中的时间范围和 API Key。
- Changed: 仪表盘趋势统计调整为 7 天及以上按天聚合与展示，统一后端分桶/补桶规则，并优化前端横轴标签显示。
- Fixed: 修复仪表盘 30 天范围下趋势图数据展示异常的问题，保证 7 天与 30 天场景的趋势图和按 Key 趋势图按天连续展示。
- Fixed: 修复仪表盘长周期趋势统计查询在 7 天与 30 天范围下返回空值的问题，统一后端时间分桶 SQL 与补桶键格式，确保请求趋势和 Token 趋势稳定返回数组数据。
...
- Changed: 同步更新 `LocalAIHub_GO/localaihub` 后端可执行文件，保持仓库内构建产物与当前代码状态一致。
- Docs: 新增 `docs/最初需求文档/` 需求资料目录，补充详细需求、PRD、开发实现、接口设计、数据库字典与手工测试模板等项目初始文档。
...
- Added: 请求日志补齐请求模型、虚拟模型、路由、上游、响应摘要、错误详情等字段，并增强日志列表展示与失败原因可读性。
- Fixed: API Key 测试支持按候选模型逐个探测并返回前序失败原因；创建后改为异步探测，同时修正配额检查与请求次数更新逻辑。
- Fixed: 默认数据库 DSN 示例切换为 `Asia/Shanghai`，并修正统计查询与趋势补桶逻辑，解决仪表盘和日志按北京时间展示时的时间窗口偏差问题。
- Changed: 管理后台补齐 `types` 类型定义，并统一优化仪表盘、日志、模型、上游、快捷流程等页面的加载态、状态切换交互与错误原因中文展示。
